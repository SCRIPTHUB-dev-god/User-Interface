...
function library:CreateTab(tabName)
	...
	function pageElements:CreateGroupBox(boxTitle, layoutType, initialState)
		-- aturan masih sama, tapi group box sesama kanan/kiri turun ke bawah sesuai side yang sama
		local isExpanded = true
		if initialState == "close" then
			isExpanded = false
		end

		-- pastikan ada container untuk left/right columns
		if not page:GetAttribute("LeftRightSetup") then
			page:SetAttribute("LeftRightSetup", true)

			-- container untuk dual columns
			local dualContainer = Instance.new("Frame")
			dualContainer.Name = "DualColumnContainer"
			...
			-- allside container untuk full width
			local allsideContainer = Instance.new("Frame")
			allsideContainer.Name = "AllsideContainer"
			allsideContainer.Size = UDim2.new(1, 0, 0, 0)
			allsideContainer.AutomaticSize = Enum.AutomaticSize.Y
			allsideContainer.BackgroundTransparency = 1
			allsideContainer.Parent = page
			allsideContainer.LayoutOrder = 2

			local allsideLayout = Instance.new("UIListLayout")
			allsideLayout.Padding = UDim.new(0, 10)
			allsideLayout.SortOrder = Enum.SortOrder.LayoutOrder
			allsideLayout.Parent = allsideContainer

			page:SetAttribute("LeftColumnReady", true)
		end
		...
		-- Tentukan parent berdasarkan side yang sama turun ke bawah
		local side = "allside"
		if type(layoutType) == "string" then
			side = string.lower(layoutType)
		end
		local parentContainer = nil
		if side == "left" or side == "right" then
			local dual = page:FindFirstChild("DualColumnContainer")
			if dual then
				if side == "left" then
					parentContainer = dual:FindFirstChild("LeftColumn")
				else
					parentContainer = dual:FindFirstChild("RightColumn")
				end
			end
			groupBox.Size = UDim2.new(1, 0, 0, isExpanded and currentCalculatedHeight or collapsedHeight)
		else
			-- allside = full width di bawah dual columns
			local allsideCont = page:FindFirstChild("AllsideContainer")
			if allsideCont then
				parentContainer = allsideCont
			else
		...
		if parentContainer then
			groupBox.Parent = parentContainer
		else
